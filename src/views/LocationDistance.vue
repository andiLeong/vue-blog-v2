<template>
    <ComponentLayout
        title="Calculate distance between 2 locations"
        class="my-20"
    >
        <form @submit.prevent="check" action="">
            <div class="space-y-8 md:space-y-0 md:grid gap-4 grid-cols-3">
                <div>
                    <input
                        v-model="target_latitude"
                        type="text"
                        class="form-input"
                        placeholder="Target location Latitude"
                    />
                </div>
                <div>
                    <input
                        v-model="target_longitude"
                        type="text"
                        class="form-input"
                        placeholder="Target location Longitude"
                    />
                </div>
        ...
        <div v-if="away">
            how many {{ away }} km from ({{ latitude }} , {{ longitude }} ) to (
